...
void processInput(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS && !irradianceKeyPressed) {
	...
	if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS && !prefilterKeyPressed) {
		prefilter = !prefilter;
		prefilterKeyPressed = true;
	}

	if (glfwGetKey(window, GLFW_KEY_3) == GLFW_RELEASE) {
		prefilterKeyPressed = false;
	}
}

void setShader() {
	Shader* pbrShader = new Shader("Shaders/PBR/pbr.vs", "Shaders/PBR/pbr.fs");
	...
	ShaderManager::instance()->get("pbrShader")->setUniform("albedo", 0.5f, 0.0f, 0.0f);
	ShaderManager::instance()->get("pbrShader")->setUniform("ao", 1.0f);
...
void drawObjects() {
	...
	{
		ShaderManager::instance()->get("pbrShader")->setUniform("metallic", (float)row / (float)nrRows);
		for (int col = 0; col < nrColumns; ++col)
		{
			// we clamp the roughness to 0.025 - 1.0 as perfectly smooth surfaces (roughness of 0.0) tend to look a bit off
			// on direct lighting.
			ShaderManager::instance()->get("pbrShader")->setUniform("roughness", glm::clamp((float)col / (float)nrColumns, 0.05f, 1.0f));
	...
	TextureManager::instance()->get("envCubeMap")->bind(0, -1, true);
	if(irradiance)
		TextureManager::instance()->get("irradianceMap")->bind(0, -1, true);	// display irradiance map
	if(prefilter)
		TextureManager::instance()->get("prefilterMap")->bind(0, -1, true);	// display prefilter map
